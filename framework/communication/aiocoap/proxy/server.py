...
class CanNotRedirectBecauseOfUnsafeOptions(CanNotRedirect):
    def __init__(self, options):
        self.code = communication.aiocoap.numbers.codes.BAD_OPTION
...
class Proxy(interfaces.Resource):
    ...
    async def render(self, request):
        ...
        try:
            ...
            return message.Message(code=communication.aiocoap.numbers.codes.GATEWAY_TIMEOUT)
...
class ProxyWithPooledObservations(Proxy, interfaces.ObservableResource):
    ...
    def _cache_key(request):
        return request.get_cache_key([communication.aiocoap.numbers.optionnumbers.OptionNumber.OBSERVE])
    ...
    def _get_observation_for(self, request):
        ...
        try:
            ...
            def eb(exception, obs=obs):
                if obs.__users:
                    code = communication.aiocoap.numbers.codes.INTERNAL_SERVER_ERROR
                    payload = b""
    ...
    async def render(self, request):
        ...
        try:
            ...
            if not isinstance(e, CanNotRedirect) and request.opt.observe is not None:
                ...
                return message.Message(code=communication.aiocoap.numbers.codes.BAD_OPTION, payload="Observe option can not be proxied without active observation.".encode('utf8'))
...
class ForwardProxy(Proxy):
    ...
    def apply_redirection(self, request):
        if request.opt.proxy_uri is not None:
            raise CanNotRedirect(communication.aiocoap.numbers.codes.NOT_IMPLEMENTED, "URI splitting not implemented, please use Proxy-Scheme.")
        if request.opt.proxy_scheme is None:
            raise CanNotRedirect(communication.aiocoap.numbers.codes.BAD_REQUEST, "This is only a proxy.") # correct error code?
        if request.opt.proxy_scheme != 'coap':
            raise CanNotRedirect(communication.aiocoap.numbers.codes.BAD_OPTION, "This is only a CoAP proxy (set uri-scheme to coap)")
...
class ReverseProxy(Proxy):
    def apply_redirection(self, request):
        if request.opt.proxy_uri is not None or request.opt.proxy_scheme is not None:
            ...
            raise CanNotRedirect(communication.aiocoap.numbers.codes.PROXYING_NOT_SUPPORTED, "This is a reverse proxy, not a forward one.")

        redirected = super(ReverseProxy, self).apply_redirection(request)
        if redirected is None:
            raise CanNotRedirect(communication.aiocoap.numbers.codes.NOT_FOUND, "")
