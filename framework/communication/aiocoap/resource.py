...
def hashing_etag(request, response):
    ...
    if response.code != communication.aiocoap.numbers.codes.CONTENT and response.code is not None:
        return
    ...
    if request.opt.etags is not None and response.opt.etag in request.opt.etags:
        response.code = communication.aiocoap.numbers.codes.VALID
        response.payload = b''
...
class Resource(_ExposesWellknownAttributes, interfaces.Resource):
    ...
    async def render(self, request):
        ...
        if response.code is None:
            if request.code in (communication.aiocoap.numbers.codes.GET, communication.aiocoap.numbers.codes.FETCH):
                response_default = communication.aiocoap.numbers.codes.CONTENT
            elif request.code == communication.aiocoap.numbers.codes.DELETE:
                response_default = communication.aiocoap.numbers.codes.DELETED
            else:
                response_default = communication.aiocoap.numbers.codes.CHANGED
