...
import communication.aiocoap.defaults
import communication.aiocoap.meta
import communication.aiocoap.proxy.client
from communication.aiocoap.util import ActionNoYes
from communication.aiocoap import media_types
...
def build_parser():
    ...
    p.add_argument('--version', action="version", version='%(prog)s ' + communication.aiocoap.meta.version)
...
def present(message, options, file=sys.stdout):
    ...
    if options.pretty_print:
        from communication.aiocoap.util import pretty_print
    ...
    color = options.color
    if color:
        from communication.aiocoap.util import lexer_for_mime
        import pygments
        try:
...
async def single_request(args, context=None):
    ...
    pretty_print_modules = communication.aiocoap.defaults.prettyprint_missing_modules()
    ...
    try:
        code = getattr(communication.aiocoap.numbers.codes.Code, options.method.upper())
    except AttributeError:
        try:
            code = communication.aiocoap.numbers.codes.Code(int(options.method))
        except ValueError:
    ...
    if options.accept:
        try:
            ...
            try:
                request.opt.accept = communication.aiocoap.numbers.media_types_rev[options.accept]
            except KeyError:
    ...
    if options.content_format:
        try:
            ...
            try:
                request.opt.content_format = communication.aiocoap.numbers.media_types_rev[options.content_format]
            except KeyError:
    ...
    if options.payload:
        if options.payload.startswith('@'):
            ...
            if communication.aiocoap.numbers.media_types.get(request.opt.content_format, "").startswith("application/cbor"):
                try:
                    import cbor
    ...
    if options.proxy is None:
        ...
        interface = communication.aiocoap.proxy.client.ProxyForwarder(options.proxy, context)
